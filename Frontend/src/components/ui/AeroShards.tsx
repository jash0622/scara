...
const advanceFormation = (state: any, flow: any, elapsed: number, duration: number, frozen: boolean) => {
...
const createRenderGraph = (gpu: any, outputSize: [number, number], bloomSize: [number, number] = resolveBloomSize(outputSize)) => {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        const outputFormat = ((navigator as any).gpu as any).getPreferredCanvasFormat();
        const output = surface(gpu, canvas, {
          dpr: resolveDpr(preset, canvas),
          autoResize: false,
          format: outputFormat
        });
        const graph = createRenderGraph(gpu, output.size as unknown as [number, number], resolveBloomSize([canvas.clientWidth, canvas.clientHeight]));
        ...
        const resizeOutput = () => {
          ...
          if (nextSize[0] !== output.size[0] || nextSize[1] !== output.size[1]) output.resize(nextSize as [number, number]);
          resizePostTargets();
        };
        ...
        const renderFrame = (currentFrame: any) => {
          ...
          configureStyle(graph, settings, output.size as unknown as number[], [canvas.clientWidth, canvas.clientHeight]);
